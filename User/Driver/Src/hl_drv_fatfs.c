    ...
    if (res_flash == FR_NO_FILESYSTEM) {
        ...
        if (res_flash == FR_OK) {
            ...
            if (res_flash == FR_OK) {
                ...
                debug_printf("[remount failed] res_remount = %d\r\n", res_flash);
            }
...
int hl_drv_fatfs_read(char* filename, uint8_t* read_buffer, uint32_t data_size)
{
    FRESULT res_flash;  //文件操作结果
    UINT     fnum;       //文件成功读写数量
...
int hl_drv_fatfs_write(char* filename, uint8_t* write_buffer, uint32_t data_size)
{
    UINT     fnum;       //文件成功读写数量
    FRESULT res_flash;  //文件操作结果
    char    path_string[20] = { FATFS_PATH };
